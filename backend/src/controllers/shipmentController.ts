...
function normalize(shipment: any) {
  return {
    ...
    route_waypoints: (shipment.routeWaypoints || []).map((wp: any) => ({
      warehouse_code: wp.warehouseCode,
      warehouse_name: wp.warehouseName,
      city: wp.city,
      region: wp.region,
      lat: wp.lat,
      lng: wp.lng,
      ...
      arrived_at: wp.arrivedAt || null,
      departed_at: wp.departedAt || null,
    })),
